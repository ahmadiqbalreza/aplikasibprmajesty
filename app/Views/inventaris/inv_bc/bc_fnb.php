...
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            <h3 class="mt-4">Inventaris Furniture Besi BPRMGR Batam Center</h3>
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            ...
                            Data Inventaris Furniture Besi
                        </div>
                        <div class="col text-end">
                            <a title='Add' onclick="add_bcfnb()" type="button" class="btn btn-primary btn-sm mr-2">Add</a>
                            <a href="#" type="button" class="btn btn-primary btn-sm mr-2">Export</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <table id="tabel_bcfnb" class="table table-striped display nowrap" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>No.</th>
                        ...
                        <tbody>
                            <?php $i = 1; ?>
                            <?php foreach ($db_bcfnb as $fnb) : ?>
                                <tr>
                                    <td><?= $i; ?></td>
                                    <td><a href="#" onclick="view_bcfnb('<?= $fnb->nomor_inventaris_fnb; ?>')"><?= $fnb->nomor_inventaris_fnb; ?><a></td>
                                    <td><?= $fnb->deskripsi; ?></td>
                                    <td><?= $fnb->jumlah_unit; ?></td>
                                    <td><?= $fnb->lokasi; ?></td>
                                    <td><?= $fnb->remark; ?></td>
                                    <td><?= $fnb->update_by; ?></td>
                                    <td><?= date('d-m-Y H:i:s', strtotime($fnb->last_update)); ?></td>
                                    <td>
                                        <a title='Detail' onclick="view_bcfnb('<?= $fnb->nomor_inventaris_fnb; ?>')" type="button" class="btn btn-info btn-sm">Detail</a>&nbsp;&nbsp;
                                        <a title='Edit' onclick="edit_bcfnb('<?= $fnb->nomor_inventaris_fnb; ?>')" type="button" class="btn btn-warning btn-sm">Edit</a>&nbsp;&nbsp;
                                        <a title='Hapus' onclick="hapus_bcfnb('<?= $fnb->nomor_inventaris_fnb; ?>')" type="button" class="btn btn-danger btn-sm">Hapus</a>
                                    </td>
                                </tr>
                                <?php $i++; ?>
                            <?php endforeach; ?>
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="modal_edit_bcfnb" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modal_edit_bcfnbLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal_edit_bcfnbLabel">Title</h5>
                </div>
                <div class="modal-body">
                    <form action="#" id="form_edit_bcfnb" name="form_edit_bcfnb" method="POST" enctype="multipart/form-data">
                        <div class="row my-2 mx-2">
                            ...
                            <div class="col-bg-6">
                                <div class="form-group">
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="nomor_inventaris_fnb" name="nomor_inventaris_fnb" autocomplete="off" disabled>
                                        <div class="invalid-feedback error_nomor_inventaris_fnb">
                            ...
                            <div class="col-bg-6">
                                <div class="form-group">
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="kategori" name="kategori" autocomplete="off" value="FNB" readonly>
                ...
                <div class="modal-footer">
                    ...
                    <button title='Simpan' onclick="simpan_fnb()" type="button" class="btn btn-primary">Simpan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Modal Detail-->
    <div class="modal fade" id="modal_detail_bcfnb" tabindex="-1" aria-labelledby="modal_detail_bcfnbLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal_detail_bcfnbLabel">Title</h5>
                </div>
                <div class="modal-body">
                    <form action="#" id="form_detail_bcfnb" name="form_detail_bcfnb">
                        <div class="row my-2 mx-2">
                                ...
                                <thead>
                                    <tr>
                                        <th colspan="3" class="text-center">
                                            <h4 class="text-uppercase fw-bold" id="nomor_inventaris_fnb" name="nomor_inventaris_fnb"></h4>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
    ...
    <script type="text/javascript">
        ...
        function add_bcfnb() {
            ...
            $('#form_edit_bcfnb')[0].reset(); // reset form on modals
            $('#modal_edit_bcfnb').modal('show'); // show bootstrap modal
        ...
        function edit_bcfnb(nomor_inventaris_fnb) {
            save_method = 'update';

            // Reset Form Validasi
            var formx = $('#form_edit_bcfnb');
            ...
            $.ajax({
                url: "<?php echo site_url('/inventaris/ajax_get_bcfnb/') ?>/" + nomor_inventaris_fnb,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    console.log(data[0]);
                    $('[name="nomor_inventaris_fnb"]').val(data[0].nomor_inventaris_fnb);
                    ...
                    $('[name="view_foto_barang"]').attr('src', "/img/inventaris/bc/fnb/" + data[0].image);
                    $('[name="remark"]').val(data[0].remark);
                    $('[name="keterangan_lain"]').val(data[0].keterangan_lain);

                    $('.modal-title').text('Edit Detail Inventaris');
                    $('#modal_edit_bcfnb').modal('show'); // show bootstrap modal when complete loaded
        ...
        function simpan_fnb() {
            var url;
            if (save_method == 'add') {
                url = "<?php echo site_url('/inventaris/bcfnb_add/') ?>";
            } else {
                url = "<?php echo site_url('/inventaris/bcfnb_update/') ?>";
            }
            // ajax adding data to database
            var nomor_inventaris_fnb = $('#nomor_inventaris_fnb').val();

            var formDatafnb = new FormData();
            // Attach file
            formDatafnb.append('inp_foto_barang', $('#inp_foto_barang')[0].files[0]);
            formDatafnb.append('nomor_inventaris_fnb', $('#nomor_inventaris_fnb').val());
            formDatafnb.append('nomor', $('#nomor').val());
            formDatafnb.append('tahun', $('#tahun').val());
            formDatafnb.append('deskripsi', $('#deskripsi').val());
            formDatafnb.append('kategori', $('#kategori').val());
            formDatafnb.append('jumlah_unit', $('#jumlah_unit').val());
            formDatafnb.append('lokasi', $('#lokasi').val());
            formDatafnb.append('lokasi_kantor', $('#lokasi_kantor').val());
            formDatafnb.append('remark', $('#remark').val());
            formDatafnb.append('keterangan_lain', $('#keterangan_lain').val());
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.error) {
                        if (data.error.nomor_inventaris_fnb) {
                            $('#nomor_inventaris_fnb').addClass('is-invalid');
                            $('.error_nomor_inventaris_fnb').html(data.error.nomor_inventaris_fnb);
                            Swal.fire({
                                icon: 'error',
                                title: 'Oops...',
                                text: data.error.nomor_inventaris_fnb
                            })
                        } else {
                            $('#nomor_inventaris_fnb').removeClass('is-invalid');
                            $('.error_nomor_inventaris_fnb').html('');
                        }
        ...
        function hapus_bcfnb(nomor_inventaris_fnb) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?php echo site_url('/inventaris/bcfnb_delete/') ?>/" + nomor_inventaris_fnb,
                        type: "POST",
                        dataType: "JSON",
                        success: function(data) {
                            Swal.fire(
        ...
        function view_bcfnb(nomor_inventaris_fnb) {
            $('#form_detail_bcfnb')[0].reset(); // reset form on modals
            <?php header('Content-type: application/json'); ?>
            //Ajax Load data from ajax
            $.ajax({
                url: "<?php echo site_url('/inventaris/ajax_get_bcfnb/') ?>/" + nomor_inventaris_fnb,
                ...
                success: function(data) {
                    console.log(data[0]);
                    $('[name="nomor_inventaris_fnb"]').text(data[0].nomor_inventaris_fnb);
                    ...
                    $('[name="image"]').attr('src', "/img/inventaris/bc/fnb/" + data[0].image);
                    ...
                    $('#modal_detail_bcfnb').modal('show'); // show bootstrap modal when complete loaded
        ...
        function cek_nomor() {
            $.ajax({
                url: "<?php echo site_url('/inventaris/ajax_get_nomor_terakhir_fnb/') ?>",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    console.log(data[0]);
        ...
        function updatee() {
            $("#nomor_inventaris_fnb").val("BC-" + $('#tahun').val() + "-FNB-" + $('#nomor').val());
        }
    </script>
